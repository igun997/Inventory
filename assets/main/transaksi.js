      ...
      dialog.init(function() {
        setTimeout(function() {
          ...
          if (cek.status == 1 && cek.data.status_transaksi == "waiting") {
            ...
            button = {type:"submit",name:"Simpan Barang",class:"primary"};
            ...
            button = {type:"submit",name:"Simpan Transaksi",class:"success"};
            ...
            dialog.find("#barang").ready(function() {
              sd = get(base_url+"api/barangget/-1/1");
              if (sd.status == 1) {
                sd_data = sd.data;
                dialog.find("#barang").append($('<option>', {value:"", text:""}));
            ...
            dialog.find("#save").on('submit', function(event) {
              ...
              if (t_keluar > current) {
                ...
                keluar[keluar.length] = {name:"total_keluar",value:current};
                if (current != 0) {
                ...
                if (ins.status == 1) {
                  ...
                  keluar[keluar.length] = {name:"total_keluar",value:t_masuk};
            ...
            dialog.find("#savetrx").on('submit', function(event) {
              ...
              dform[dform.length] = {name:"id_transaksi_barang_keluar",value:idtrx};
              var tb = parseFloat(dialog.find("#tb").val());
              var bayar = parseFloat(dialog.find("#total_bayar").val());
              i = post(base_url+"api/akuntansave",{id_transaksi_barang_keluar:idtrx,tipe:"pemasukan",total:bayar,alasan:"Pembayaran"});
              if (i.status != 1) {
                swal("Error","Kesalahan di Akuntan","error");
              }else {
                if (bayar < tb) {
                  dform[dform.length] = {name:"status_transaksi",value:"cashbon"};
                  dform[dform.length] = {name:"cashbon",value:(tb-bayar)};
                  ...
                  dform[dform.length] = {name:"cashbon",value:"0"};
                  up = post(base_url+"api/transaksiupdate",dform);
                  if (up.status == 1) {
                    bootbox.hideAll();
                    table_main.ajax.reload();
                ...
                dialog.init(function() {
                  setTimeout(function() {
                        ...
                        dialog.find("#brg").ready(function() {
                          ...
                          if (sd.status == 1) {
                            ...
                            for (var i = 0; i < sd_data.length; i++) {
                              ...
                              dialog.find("#brg").append($('<option>', {value:sd_data[i].id_barang, text:sd_data[i].nama_barang+" - Stok = "+sd_data[i].stok}));
                            }
                          }else {
                            bootbox.hideAll();
                            swal("Error","Data Tidak Ada ","error");
                        ...
                        dialog.find("#upedit").on('submit',function(event) {
                          ...
                          if (up.status == 1) {
                            ...
                            if (cur > old && total_stok > cur) {
                              ...
                              post(base_url+"api/barangupdate",{id_barang:gedit.data.id_barang,stok:plus});
                            }else if (cur < old){
                              plusuk = (old - cur);
                              plusuk = (plusuk + total_stok);
                              console.log("Penambahan Stok "+ plusuk);
                              post(base_url+"api/barangupdate",{id_barang:gedit.data.id_barang,stok:plusuk});
                            }
                            var total = get(base_url+"api/barangkeluarcalc/"+idtrx);
                            $("#tb").val(total.total);
                            getb();
                    ...
                    dialog.find(".hapus").on('click', function(event) {
                      ...
                      if (con) {
                        var del = post(base_url+"api/barangkeluardelete",{id_barang_keluar:id});
                        if (del.status == 1) {
                          dialog.modal("hide");
                          var total = get(base_url+"api/barangkeluarcalc/"+idtrx);
                          $("#tb").val(total.total);
                ...
                dialog.init(function() {
                  setTimeout(function() {
                    ...
                    dialog.find(".hapus").on('click', function(event) {
                      ...
                      var con = confirm("Apakah Anda Yakin ? ");
                      if (con) {
                        var del = post(base_url+"api/barangkeluarpreorderdelete",{id_transaksi_barang_keluar_preorder:id});
                        if (del.status == 1) {
                          dialog.modal("hide");
                          swal("Sukses","Data Di Hapus","success");
                          var total = get(base_url+"api/barangkeluarcalc/"+idtrx);
            ...
            button = {type:"submit",name:"Bayar",class:"success"};
            ...
            dialog.find("#byar").on('submit', function(event) {
              ...
              if (ba == parseFloat(cek.data.cashbon)) {
                post(base_url+"api/transaksiupdate",{id_transaksi_barang_keluar:cek.data.id_transaksi_barang_keluar,cashbon:"0",total_bayar:ta,status_transaksi:"lunas"});
                ...
                tsb = (tbs + ba);
                post(base_url+"api/transaksiupdate",{id_transaksi_barang_keluar:cek.data.id_transaksi_barang_keluar,cashbon:sisa,total_bayar:tsb});
              }
    ...
    $("#add").on('click', function(event) {
      ...
      dialog.init(function() {
        setTimeout(function() {
          ...
          button = {type:"submit",name:"Lanjutkan",class:"primary"};
          html = builder(input,button,"save");
          ...
          dialog.find("#save").on('submit', function(event) {
            ...
            dform = $(this).serializeArray();
            timestamp = $.now();
            dform[dform.length] = {name:"nofaktur",value:timestamp};
